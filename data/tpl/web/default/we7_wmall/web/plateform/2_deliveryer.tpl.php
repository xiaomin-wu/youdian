...
<div class="clearfix">
	...
	<form class="form-horizontal" action="" method="post" id="form-deliveryer">
		<div class="panel panel-default">
			<div class="panel-body table-responsive">
				<table class="table table-hover">
					<thead class="navbar-inner">
						...
						<tr>
							<td>
								<label>
									...
									&nbsp;
									<img src="<?php  echo tomedia($item['deliveryer']['avatar']);?>" width="48">
								</label>
							</td>
					...
					<tr>
						<td colspan="8">
							<input type="checkbox" id="select-all" data-scope="#form-deliveryer">
							&nbsp;
...
$(function(){
	...
	$(document).on('click', '.account-turncate-item', function(){
		...
		$(document).on('click', '.account-turncate-submit', function(){
			...
			$.post("<?php  echo $this->createWebUrl('ptfdeliveryer', array('op' => 'account_turncate'));?>", {ids: ids, remark: remark}, function(data){
				var result = $.parseJSON(data);
...
<div class="clearfix">
	...
	<form class="form-horizontal" action="" method="post" id="">
		<ul class="order-nav order-nav-tabs">
			...
			<li <?php  if($trade_type == 3) { ?>class="active"<?php  } ?>><a href="<?php  echo filter_url('trade_type:3');?>">其他变动</a></li>
		</ul>
		<div class="panel panel-default">
			<div class="panel-body table-responsive">
				<table class="table table-hover">
					...
					<tr>
						...
						<td align="right">
							<?php  if($record['status'] != 1) { ?>
								...
								<a href="<?php  echo $this->createWebUrl('ptfdeliveryer', array('op' => 'gatcashstatus', 'id' => $record['id'], 'status' => 1));?>" onclick="if(!confirm('确定变更提现状态吗')) return false;" class="btn btn-default btn-sm">设为已处理</a>
							<?php  } ?>
						</td>
					</tr>
					<?php  } } ?>
