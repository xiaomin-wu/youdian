						...
						<ul>
							<li>
								<a href="<?php  echo $this->createMobileUrl('errander-order');?>" class="item-link item-content">
									<div class="item-media"><img src="<?php echo MODULE_URL;?>resource/app/img/suiyigou_icon.png" alt=""/></div>
						...
						<div class="order-btn table border-1px-t">
							...
							<?php  if($order['status'] == 1) { ?>
								...
								<?php  if($order['is_pay'] == 1) { ?>
									<a href="javascript:;" class="order-remind table-cell border-1px-l" data-id="<?php  echo $order['id'];?>">催单</a>
								<?php  } ?>
							...
							<?php  } else if(in_array($order['status'], array(5))) { ?>
								<a href="<?php  echo $this->createMobileUrl('goods', array('f' => '1', 'id' => $order['id'], 'sid' => $order['sid']));?>" class="table-cell border-1px-r" data-id="<?php  echo $order['id'];?>">再来一单</a>
