		...
		<form action="" method="post" id="form-account">
			<div class="list-block">
				<ul class="border-1px-tb">
					...
					<?php  if($config_settle['mobile_verify_status'] == 1) { ?>
						<li>
...
<script>
$(function(){
	var $mobile_verify_status = "<?php  echo $config_settle['mobile_verify_status'];?>";
	if($mobile_verify_status == 1) {
		$('#btn-code').click(function(){
			...
			$.post("<?php  echo $this->createMobileUrl('cmncode', array('sid' => 0, 'product' => $_W['we7_wmall_plus']['config']['title'] . '商户入驻'))?>", {mobile: mobile}, function(data){
				if(data != 'success') {
					$.toast(data);
				} else {
					$.toast('验证码发送成功, 请注意查收');
...
<div class="page business-enter">
	...
	<div class="content">
		<form action="" method="post" id="form-store">
			...
			<?php  if(!empty($config_settle['agreement'])) { ?>
...
<div class="popup popup-settle-agreement">
	<div class="page">
		...
		<div class="content" style="background: #FFF">
			<div class="content-padded">
				<?php  echo $config_settle['agreement'];?>
			</div>
		</div>
	</div>
</div>
