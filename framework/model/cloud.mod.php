...
function cloud_m_prepare($name) {
	...
	$dat = cloud_request('http://www.010xr.com', $pars);
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://www.010xr.com', $pars);
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://www.010xr.com', $pars, $headers, 300);
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://www.010xr.com', $pars);
...
function cloud_auth_url($forward, $data = array()){
	...
	$auth_url = 'http://www.010xr.com/web/index.php?c=auth&a=passwort&__auth=' . $query;
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://www.010xr.com', $pars, array(), 300);
